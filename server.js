...
function getStep(value){ return value >= 200 ? 10 : 5; }
...
// Start next auction but wait for host spin
function prepareNextAuction(roomId){
  ...
  if(!picked){
    io.to(roomId).emit('chat', { msg: `No more players for ${pos}. Skipping.` });
    return setTimeout(()=> prepareNextAuction(roomId), 500);
  }
  ...
  room.currentAuction = {
    ...
    initialTimer: null,
    postBidTimer: null,
    timeLeft: 60,   // initial draft timer
    waitingForSpin: true  // wait host spin
  };

  io.to(roomId).emit('auctionReady', publicAuction(room.currentAuction, room));
}
  ...
  auction.initialTimer = setInterval(()=>{
    ...
    if(ticks <= 0){
      ...
      if(!auction.highestBidder){
        io.to(roomId).emit('chat', { msg: 'No bids. Player skipped.' });
        room.soldPlayers.add(auction.player.name);
        room.currentAuction = null;
        setTimeout(()=> prepareNextAuction(roomId), 1500);
      } else {
        auction.timeLeft = 30; // per-bid timer
        auction.postBidTimer = setTimeout(()=> finalizeAuction(roomId), 30*1000);
        io.to(roomId).emit('chat', { msg: 'First bid done. 30s per-bid timer started.' });
      }
    }
  },1000);
}
...
function finalizeAuction(roomId){
  ...
  if(auction.highestBidder){
    ...
    if(winner.balance >= price && winner.team.length < 11){
      ...
      io.to(roomId).emit('chat', { msg: `Could not finalize sale to ${winner.username}` });
    }
  } else {
...
io.on('connection', socket=>{
  ...
  socket.on('createRoom', ({roomId, capacity, username}, cb)=>{
    ...
    rooms[roomId]={
      id: roomId, capacity, players:{}, order:[], host: socket.id,
      phase:'lobby', soldPlayers:new Set(), currentAuction:null
    };
    socket.join(roomId);
    rooms[roomId].players[socket.id]={id:socket.id,username,balance:1000,team:[],skipped:false};
  ...
  socket.on('joinRoom', ({roomId, username}, cb)=>{
    ...
    socket.join(roomId);
    room.players[socket.id]={id:socket.id,username,balance:1000,team:[],skipped:false};
  ...
  socket.on('startGame', ({roomId})=>{
    ...
    room.phase='running';
    io.to(roomId).emit('roomUpdate', sanitizeRoom(room));
    prepareNextAuction(roomId);
  });

  socket.on('spinWheel', ({roomId})=>{
    const room = rooms[roomId];
    if(!room || !room.currentAuction || !room.currentAuction.waitingForSpin) return;
    room.currentAuction.waitingForSpin = false;
    io.to(roomId).emit('auctionStart', publicAuction(room.currentAuction, room));
  ...
  socket.on('bid', ({roomId, amount}, cb)=>{
    ...
    if(player.skipped) return cb({ok:false,msg:'You skipped'});

    const step = getStep(auction.currentBid);
    if(!auction.highestBidder){
      if(amount!==auction.basePrice && amount!==auction.basePrice+step) return cb({ok:false,msg:'First bid must be basePrice or basePrice+step'});
    } else {
